...
- **Naive mean of means**: micro-batch 1 computes `sum_1 / 10`, micro-batch 2 computes `sum_2 / 100`, then the accumulated gradient is `(sum_1/10 + sum_2/100) / 2`. Each token in the 10-token batch has per-token weight `1/20` while each token in the 100-token batch has per-token weight `1/200` which is a 10x imbalance.
- **True global mean**: the gradient is `(sum_1 + sum_2) / 110`. Every token contributes equally with per-token weight `1/110`.

This issue is also discussed in [Unsloth (gradient accumulation bug)](https://unsloth.ai/blog/gradient) and [Tulu 3 (Section 4.3.2)](https://arxiv.org/abs/2411.15124).
