...
[
\mathcal{L}*{\text{GRPO}}(\theta)=-\mathbb{E}\Big[\min\big(rA,\ \mathrm{clip}(r,1-\epsilon*\ell,1+\epsilon_h)A\big)\Big].
]
CISPO instead clips the ratio and uses the **clipped ratio as a stop-gradient coefficient** multiplying the log-prob term:
[
\mathcal{L}*{\text{CISPO}}(\theta)= -\mathbb{E}\Big[\mathrm{sg}!\big(\mathrm{clip}(r,1-\epsilon*\ell,1+\epsilon_h)\big)\ \log p_\theta(\cdot)\ A\Big].
]
