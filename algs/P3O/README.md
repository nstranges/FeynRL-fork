...
- **Same loss in sync and async; no decoupled loss needed.** P3O's `compute_policy_loss` runs **unchanged** between sync and overlap mode: ESS is computed from each batch's own importance weights and adaptively controls both the ratio cap and the trust-region KL weight, so no separately-tuned staleness coefficient is required. This is one of the main features of P3O.

  This is a structural difference from the other RL algorithms in this repo. PPO / GRPO / CISPO use a **fixed** clip range and therefore auto-enable the [decoupled loss](../GRPO/README.md#decoupled-loss-overlap-mode) in overlap mode (proximal snapshot $\pi_{\mathrm{prox}}$ computed once per shard, plus a behavioral importance weight $w = \pi_{\mathrm{prox}}/\pi_{\mathrm{old}}$ with an optional `overlap.behave_imp_weight_cap`). P3O needs none of that machinery: no proximal snapshot, no behavioral IS correction, and no re-tuning of clip bounds. The `use_decoupled_loss` flag wired from `overlap.enabled` in [`core/rl_engines.py:56`](../../core/rl_engines.py#L56) is consumed by the other algorithms; P3O's constructor accepts it for signature compatibility but treats it as a no-op.
- **Global token normalization instead of per-micro-batch means.** We compute the global valid-token count across all micro-batches and all ranks before the training loop and use it as the loss denominator (enabled via `normalize_loss=True`). This avoids the "mean of means ≠ global mean" bias that arises with per-micro-batch means (`loss_sum / mask.sum()`) under variable-length rollouts, so every action token contributes equally to the gradient regardless of which micro-batch or rank it lands on. See [RL Common README — Global Token Normalization](../RL/README.md#global-token-normalization-for-rl) and [SFT README — Loss Normalization](../SFT/README.md#loss-normalization) for the derivation.
