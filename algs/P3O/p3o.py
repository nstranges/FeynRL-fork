...
class P3O(COMMON):
    def __init__(self,
        ...
        # params for decoupled ppo clipped loss
    ...
    def compute_policy_loss(self,
        ...
        if self.use_decoupled_loss:
            ...
            ess_factor        = self.calculate_ess(ratio=r_prox, mask_bool=mask_bool)

            # Behavioral IS weight: w = pi_prox / pi_behav
            # Per-sequence ESS cap: staleness is a per-sequence property (each
            # sequence was generated by a specific policy version), so each
            # sequence gets its own adaptive cap via seq_ess [B, 1].
            ...
            # seq_ess_cap: [B, 1]
            seq_ess_cap = self.calculate_seq_ess(ratio=behave_w, mask_bool=mask_bool)  # behave_w: [B, T]
            behave_w = torch.minimum(behave_w, seq_ess_cap).clamp(min=0.0)

            # 3. P3O loss: rho_prox.detach() * log(pi) * A * w
            rho = torch.clamp(r_prox, min=0.0, max=ess_factor)
            pi_sum = -(rho.detach() * logprobs * adv * behave_w * mask).sum()
