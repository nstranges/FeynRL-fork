...
def test_ppo_init_and_engine():
    ...
    with patch.object(PPO, 'load_model') as mock_load:
        ...
        value_model = MagicMock(spec=torch.nn.Module)
        value_model.parameters.return_value = [torch.nn.Parameter(torch.randn(1), requires_grad=True)]
        ...
        assert ppo.ready is True
        assert ppo.alg_name == "PPO"
        assert deepspeed.initialize.call_count >= 2 # policy and value
        ...
        micro_batches = [
            {
                ...
                'pred_zscores': torch.zeros(1, 4),
            }
        ]
        
        # Mock precompute_gae to avoid nested method call complexity in this unit test
        ppo.precompute_gae = MagicMock(return_value=[(torch.zeros(1, 3), torch.zeros(1, 3))])
        ...
        ppo.compute_policy_loss = MagicMock(return_value=(torch.tensor(1.0, requires_grad=True), {'clipfrac': 0.1, 'approx_kl': 0.01, 'kl_ref': 0.0, 'ent_loss': 0.0, 'pi_loss': 1.0, 'pi_loss_total': 1.0}))
        ...
        ppo.policy_engine.gradient_accumulation_steps = MagicMock(return_value=1)
        
        metrics = ppo.train_step(engine_id=0, micro_batches=micro_batches)
        
        assert 'pi_loss' in metrics
        assert 'value_loss_v' in metrics
        assert ppo.policy_engine.backward.called
        assert ppo.value_engine.backward.called
