...
class PromptResponseDataset(Dataset):
    ...
    def __init__(self, 
        ...
        assert rejected_answer_key != "", "rejected_answer_key cannot be empty"
        ...
        self.rejected_answer_key = rejected_answer_key
    ...
    def __getitem__(self, idx):
        ...
        rejected_answer = current_sample[self.rejected_answer_key]
        ...
        # rejected_answer cannot be empty
        if not rejected_answer or (isinstance(rejected_answer, str) and rejected_answer.strip() == ""):
            raise ValueError(f"Sample {current_sample}: Rejected answer cannot be empty or whitespace-only")
        ...
        # 4. Tokenize rejected answer + add EOS
        rejected_answer_ids_output = self.tokenizer(rejected_answer,
                                                   return_tensors='pt',
                                                   add_special_tokens=False)
        eos_tensor = torch.tensor([self.tokenizer.eos_token_id],
                                   dtype=rejected_answer_ids_output['input_ids'].dtype)
        rejected_answer_ids = torch.cat([rejected_answer_ids_output['input_ids'][0], eos_tensor], dim=0)
        rejected_answer_attn_mask = torch.cat([rejected_answer_ids_output['attention_mask'][0],
                                               torch.tensor([1], dtype=rejected_answer_ids_output['attention_mask'].dtype)], dim=0)

        # Validate rejected answer has at least one token besides EOS
        if len(rejected_answer_ids) <= 1:
            raise ValueError(
                f"Sample {idx}:{current_sample}: Rejected answer must tokenize to at least one token (excluding EOS). "
                f"Got {len(rejected_answer_ids)} tokens total."
            )
